...
describe('CourseServiceEndToEnd', () => {
    ...
    it('modifyCourse() should modify the course', async () => {
        const course = getFakeCourse();
        await courseModel.create(course);
        const modifiedCourse = { ...course, name: 'Modified Name' };
        await service.modifyCourse(modifiedCourse);
        const courseFromDB = await courseModel.findOne({ subjectCode: course.subjectCode });
        expect(courseFromDB.name).toEqual('Modified Name');
    });
