...
import { SHRINE_BUFF_DURATION, SHRINE_BUFF_MULTIPLIER, SHRINE_TURN_LEFT, HALF_DOUBLE_NOTHING } from '@common/types/game.constant';
...
export function giveShrineBuff(game: Game, player: Player, payload: ActionOnTilePayload): void {
    ...
    const shrine = game.shrine.find(findShrine => findShrine.position
        .some(findPlayer => findPlayer.x === payload.position.x && findPlayer.y === payload.position.y));
